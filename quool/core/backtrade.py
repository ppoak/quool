...
def evaluate(
    ...
    down_volatility = returns.loc[returns < 0].std() * np.sqrt(252) * 100
    ...
    def split_dividend(self):
        ...
        for data in self.datas:
            ...
            if size and not np.isnan(divfactor):
                ...
                self.log(f"{self.data._name} dividend cash: {dividend:.2f}")
            ...
            if size and not np.isnan(splitfactor):
                ...
                self.log(f"{self.data._name} split shares: {splitsize:.0f}")
